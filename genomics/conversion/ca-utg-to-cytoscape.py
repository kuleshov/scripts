...
with open(args.asm) as asm:
	...
	for line in asm:
		if line.startswith('{'):
			...
			if braces_stack == 1:
				if message_type == 'UTG' or message_type == 'ULK':
...
for message_type, message_contents in messages:
	...
	elif message_type == 'ULK':
		m = link_ids_re.search(message_contents)
		ut1, ut2 = m.group(1), m.group(2)
		links.append((ut1, ut2))

connections = open(args.connections, 'w')
for ut1, ut2 in links:
