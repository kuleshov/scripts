...
for i in xrange(num_reads):
	...
	r_vector = np.random.random(size=len(read))
	err_positions = [i for (i,x) in enumerate(r_vector) if x < error_probability]
	for pos in err_positions:
		read[pos] = random.choice(alphabet)
	...
	qual.write(' '.join([q_str] * len(read)) + '\n')
